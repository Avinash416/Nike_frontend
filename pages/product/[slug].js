...
const ProductDetails = ({ product, products }) => {
    ...
    const isInWishlist = wishlistItems.some((item) => item.id === product?.data?.[0].id);
    ...
    const handleWishlistClick = () => {
        if (isInWishlist) {
            dispatch(removeFromWishlist(product?.data?.[0].id));
    ...
    const handleAddToCart = () => {
        if (!selectedSize) {
            ...
            if (isInWishlist) {
                dispatch(removeFromWishlist(product?.data?.[0].id));
                notify("Product removed from wishlist and added to cart!");
            } else {
